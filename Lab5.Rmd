...
The cases in this dataset are each recorded birth, there are 1000 cases in our sample
...
It highlights that on average babies that can from non smoker parents are slightly heavier
...
`length`.
Sample size conditions are met because each group has over 30 samples
...
inference(y = nc$weight, x = nc$habit, est = "median", type = "ht", null = 0, 
...
inference(y = nc$weight, x = nc$habit, est = "median", type = "ci", null = 0, 
          alternative = "twosided", method = "simulation", 
...
```{r}
inference(y = nc$weight, x = nc$mature, est = "median", type = "ht", null = 0, 
          alternative = "twosided", method = "simulation", order = c("younger mom","mature mom"))
```
...
Research Question: Do the mother’s weeks to birth has an incidence on low birth weight?

```{r}
inference(y = nc$weeks, x = nc$lowbirthweight, est = "median", type = "ht", null = 0, 
          alternative = "twosided", method = "simulation")
```
