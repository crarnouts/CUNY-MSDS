---
title: "Data 606 - week 2 - lab One - Introduction to data"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
#There are 20000 cases and there are 9 varaibles
  genhealth - categorical (ordinal)
  exerany - categorical (boolean)
  hlthplan - categorical (boolean)
  smoke100 - categorical (boolean)
  height - discrete
  weight - discrete 
  wtdesire - discrete
  age - discrete
...
heavy_and_healthy <- subset(cdc, bmi>50 & genhlth == "excellent")
heavy_and_healthy
```
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    #The plot below suggests females have an overall lower BMI on average and also have a higher amount of variability of BMI depending on the individual
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    #generally the taller the person the heavier they are but there is quite a lot of variability within this relationship
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    #wdiff basically indicates the difference between someone's actual weight and their desired weight if wdiff is greater than zero that means that their actual weight is higher than their desired weight
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    #the average person is about 15 lbs above their desired weight and most people are pretty tightly clustered in this slightly overweight category, looking at this I could assume that most people think they are overweight 
```{r}

cdcss<-subset(cdc, wdiff>-100)
plot(cdcss$wdiff)
mean(cdcss$wdiff)
hist(cdcss$wdiff, breaks = 50)
boxplot(cdcss$wdiff)
```
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    #women tend to be slightly more overweight than men
```{r}

summary(cdc$wdiff)
cdcfemale<-subset(cdc, gender=="f")
cdcmale<-subset(cdc, gender == "m")
summary(cdcfemale$wdiff)
summary(cdcmale$wdiff)

boxplot(cdcss$wdiff ~ cdcss$gender)
```
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    #70% of the cases are within one stddev of the mean for weight
...
cdcweightstddev <- subset(cdc, weight>129.7&weight<209.7)
weightproponestddev <- length(cdcweightstddev$weight)/length(cdc$weight)
weightproponestddev
```
