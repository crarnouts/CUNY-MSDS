...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    #the male distribution is much higher than the female distribution
    #the female distribution has a higher spread or variance then the male data
    
```{r}
library(ggplot2)
ggplot(fdims,aes(x=hgt)) + geom_histogram()
ggplot(mdims,aes(x=hgt)) + geom_histogram()
plot <- ggplot(bdims,aes(x=bdims$hgt,color = sex)) + geom_histogram(fill ="white")
plot
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
  ...
  #based on the plot above it appears the data does follow a nearly normal pattern
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
    #the simulated probability plot has more variance in boundary quanitles while the actual probability plot has more #variance throughout the probability plot
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    #Yes the normal probability plot for the hgt data looks similar to the plots created for the simulation data this does provide evidence that the female h

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    # The female weights seem to be much more spread out than their heights and there are some outliers that make the #distribtuion more right skewed
    
```{r}
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)
sim_norm_weight <- rnorm(n = length(fdims$wgt), mean = fwgtmean, sd = fwgtsd)
qqnorm(sim_norm_weight)
qqline(sim_norm_weight)
qqnorm(fdims$wgt)
qqline(fdims$wgt)
qqnormsim(fdims$wgt)
ggplot(fdims,aes(x=wgt)) + geom_histogram()
```
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    #the height variable had a closer agreement between the two methods meaning the height variable is more \
    #normally distributed
    
    female heights between 100 and 150 cm
```{r}
#using the normal distribution
pnorm(q = 150, mean = fhgtmean, sd = fhgtsd)- pnorm(q = 100, mean = fhgtmean, sd = fhgtsd)
#solving for it empirically
(sum(fdims$hgt < 150)-sum(fdims$hgt < 100)) / length(fdims$hgt)
```

female weights between 50 and 70 kg
```{r}
#using the normal distribution
pnorm(q = 70, mean = fwgtmean, sd = fwgtsd)- pnorm(q = 50, mean = fwgtmean, sd = fwgtsd)
#solving for it empirically
(sum(fdims$wgt < 70)-sum(fdims$hgt < 50)) / length(fdims$wgt)
```
...
```{r}
ggplot(fdims,aes(x=bii.di)) + geom_histogram()
qqnorm(fdims$bii.di)
qqline(fdims$bii.di)
```
...
```{r}
ggplot(fdims,aes(x=elb.di)) + geom_histogram()
qqnorm(fdims$elb.di)
qqline(fdims$elb.di)
```
    ...
    **c.** The histogram for general age (`age`) belongs to normal probability 
    plot letter ____.
```{r}
ggplot(fdims,aes(x=age)) + geom_histogram()
qqnorm(fdims$age)
qqline(fdims$age)
```
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter ____.
```{r}
ggplot(fdims,aes(x=che.de)) + geom_histogram()
qqnorm(fdims$che.de)
qqline(fdims$che.de)
```
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    #age has a step wise pattern because it is a discrete variable there aren't any ages that have decimal places in them
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    #the variable is left skewed 
```{r}
ggplot(fdims,aes(x=kne.di)) + geom_histogram()
qqnorm(fdims$kne.di)
qqline(fdims$kne.di)
```
