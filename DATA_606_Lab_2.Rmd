---
title: "Lab 2 Probability - Corey Arnouts"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    came up heads?
    # 19 came up with the result of heads
```{r}
...
-   Describe the distribution of streak lengths. What is the typical streak 
  ...
  #The simulated player's longest streak in 133 shots is 5 baskets
  the typical streak length for the simulated player is one or two baskets
...
-   If you were to run the simulation of the independent shooter a second time, 
  ...
  #The distributions would look similar for the most part with some slight variations that are just due to natural variation in the simulation. 
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
  ...
  #No I would say that Kobe's shooting does not fit the mold of the "hot hand model", if anything the simulation with the same overall hit percentage actually has more high hit streaks then kobe does, Kobe's "hot hand" is most likely just due to variance. Thinking about this in a holistic sense the arguement whether hot hands are due to an athlete actually being in the "zone" or just due to natural variance may be two in the same, human's have natural variance in performance/emotion that may be what causes the "hot hands" phenomonem.
